...
class RatingScreen extends Component {
  ...
  getNonce() {
    fetch(
      ...
      }/api/get_nonce/?controller=reactappsettings&method=react_create_product_review`,
      {
        method: 'GET'
      }
    )
  ...
  canBeSubmitted() {
    const { email, name, rating, description } = this.state;
    return (
      email.length > 0 &&
      name.length > 0 &&
      rating > 0 &&
  ...
  render() {
    ...
    return this.state.activityIndicatorTemp ? (
      ...
      <View style={styles.container}>
        ...
        <View
          style={{ backgroundColor: themeStyle.backgroundColor, padding: 8, flexDirection: 'row' }}
        >
          <Icon
            name={'person'}
            style={{ color: themeStyle.otherBtnsColor, fontSize: 35, paddingRight: 5, paddingLeft: 5 }}
          />
          <Text
            style={{
              fontSize: 22,
        ...
        <FlatList
          data={
            this.state.reviews !== undefined && this.state.reviews !== null && this.state.reviews.toString() !== 'NaN'
        ? this.state.reviews : []}
