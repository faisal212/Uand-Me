...
class Login extends Component {
  ...
  login = t => {
    ...
    fetch(
      ...
      }/api/reactappusers/react_generate_cookie/?insecure=cool`,
      {
        method: 'POST',
        body: formData
      }
      ...
      .then(res => res.json())
      .then(data => {
        if (data.status === 'ok' && data.cookie !== '' && data.cookie !== undefined && data.cookie !== null) {
  ...
  createAccount(info, type) {
    ...
    fetch(
      `${
        this.props.isLoading.Config.url
      }/api/reactappusers/react_fb_connect/?insecure=cool&access_token=${info}`,
      {
        method: 'GET'
      }
    )
        ...
        >
          ...
          >
            <TextInput
              ...
              placeholder={this.props.isLoading.Config.languageJson['Email or Username']}
